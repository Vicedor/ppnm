...
public static class ComplexIntegrate {
	...
	public static (complex, complex) IntegrateIterator(Func<complex, complex> f, complex a, complex b,
		...
		complex err = Q - q;
		if (magnitude(err) <= Max(acc, magnitude(Q) * eps)) {
			return (Q, err);
		}
		else {
			(complex r1, complex e1) = IntegrateIterator(f, a, (a+b)/2, f1, f2, acc/Sqrt(2), eps);
			(complex r2, complex e2) = IntegrateIterator(f, (a+b)/2, b, f3, f4, acc/Sqrt(2), eps);
	...
	public static (complex, complex) Integrate(Func<complex, complex> f, complex a, complex b,
	...
	public static (complex, complex) VT_Integrate(Func<complex, complex> f, complex a, complex b,
			                              double acc=0.001, double eps=0.001) {
	...
	public static (complex, complex) Integrate(Func<double, complex> f, double a, double b,
						   double acc=0.001, double eps=0.001) {
	...
	public static (complex, complex) VT_Integrate(Func<double, complex> f, double a, double b,
						      double acc=0.001, double eps=0.001) {
		Func<complex, complex> ft = z => f(z.Re);
	...
	// Transform of an integral of with infinite limits 
